...
function checkCasinoSpend(amount, what) {
    ...
    if (player.coins < amount) { alert(`靈石不足！${what}需要 ${amount.toLocaleString()} 靈石（目前 ${player.coins.toLocaleString()}）。`); return false; }
    if (amount > getCasinoRemaining()) {
        alert(`已接近今日下注上限！\n今日上限 ${getCasinoDailyLimit().toLocaleString()}（依境界），還能下注 ${getCasinoRemaining().toLocaleString()} 靈石。\n明天再來，或提升境界提高上限。`);
        return false;
    }
    if (amount >= player.coins * CASINO_CONFIRM_RATIO &&
        !confirm(`⚠️ ${what}要花費 ${amount.toLocaleString()} 靈石，佔你目前靈石的 ${Math.round(amount / player.coins * 100)}%。\n確定要下注嗎？`)) return false;
    return true;
}
...
function grantStoneOutcome(o) {
    ...
    switch (o.type) {
        ...
        case 'coins':
            player.coins += n;
            return { text: `💰 石中夾著靈石 ×${n.toLocaleString()}`, plain: `靈石 ×${n.toLocaleString()}`, value: n };
        case 'shards':
            addIronShards(n);
...
function cutStone(stoneId, count) {
    ...
    addLog(`☄️ 天星賭坊切開 ${count} 顆${stone.name}（${cost.toLocaleString()} 靈石）：${summary}`, results.some(r => r.rare) ? "level-up" : "system");
    checkTitleUnlocks();
    updateUI();

    let listHtml = results.map(r => `<div class="casino-line${r.rare ? ' rare' : r.value === 0 ? ' waste' : ''}">${r.text}</div>`).join('');
    let footer = `<div class="casino-sum">花費 ${cost.toLocaleString()}｜估值 ${gain.toLocaleString()}（${gain >= cost ? '<b style="color:#4ade80;">賺</b>' : '<b style="color:#f87171;">虧</b>'} ${Math.abs(gain - cost).toLocaleString()}）</div>`;
...
function rollDice() {
    ...
    if (bet < CASINO_DICE_MIN_BET) { alert(`最低押注 ${CASINO_DICE_MIN_BET.toLocaleString()} 靈石。`); return; }
    if (bet > getDiceMaxBet()) { alert(`單把最多押 ${getDiceMaxBet().toLocaleString()} 靈石（每日上限的 ${Math.round(CASINO_DICE_MAX_RATIO * 100)}%）。`); return; }
    ...
    recordCasino(bet, back, win ? `擲骰【${describeDiceBet()}】贏 ${(back - bet).toLocaleString()}` : '');
    let sum = dice[0] + dice[1] + dice[2];
    let faces = dice.map(n => CASINO_DICE_FACES[n - 1]).join(' ');
    addLog(`🎲 天星賭坊擲骰 ${faces}（${sum} 點）：押【${describeDiceBet()}】${bet.toLocaleString()} → ${win ? `贏 ${(back - bet).toLocaleString()} 靈石！` : '輸了。'}`, win ? "level-up" : "system");
    checkTitleUnlocks();
    updateUI();
    ...
    let timer = setInterval(() => {
        ...
        if (frame >= 8) {
            ...
            casinoResultHtml = `<div class="casino-dice">${faces}</div>
                <div class="casino-line">${sum} 點${triple ? '（豹子！）' : sum >= 11 ? '（大）' : '（小）'}｜押【${describeDiceBet()}】${bet.toLocaleString()}</div>
                <div class="casino-line ${win ? 'rare' : 'waste'}">${win ? `🎉 贏了！拿回 ${back.toLocaleString()}（淨贏 ${(back - bet).toLocaleString()}）` : `💸 莊家通殺，輸 ${bet.toLocaleString()}`}</div>`;
            casinoBusy = false;
            renderCasino();
        }
    }, 70);
...
function renderCasino() {
    ...
    let head = `<p class="casino-head">持有靈石 <b>${player.coins.toLocaleString()}</b>｜今日已下注 ${st.wagered.toLocaleString()} / ${limit.toLocaleString()}
        <br><small>今日輸贏（估值）<b style="color: ${st.net >= 0 ? '#4ade80' : '#f87171'};">${st.net >= 0 ? '+' : ''}${st.net.toLocaleString()}</b>${st.bestText ? `｜今日最大收穫：${st.bestText}` : ''}</small></p>`;
...
function renderCasinoStones() {
    ...
    return `<div class="grid-container">${casinoStones.map(s => {
        ...
        let table = s.odds.map(o => {
            ...
            return `<span>${label}${o.amount ? ` ${o.amount[0].toLocaleString()}～${o.amount[1].toLocaleString()}` : ''} <b>${o.w}%</b></span>`;
        }).join('');
        return `<div class="card casino-stone">
            <h3>${s.icon} ${s.name}</h3>
            <p style="font-size: 0.8em; color: #9ca3af;">${s.desc}</p>
            <p style="color: var(--accent);">每顆 ${s.price.toLocaleString()} 靈石</p>
...
function renderCasinoDice() {
    ...
    return `<div class="casino-bets">${types}</div>${extra}
        <div class="casino-amount">
            ...
            <small>單把 ${CASINO_DICE_MIN_BET.toLocaleString()}～${max.toLocaleString()}｜押【${describeDiceBet()}】中了拿回 ${(casinoDice.amount * (payout + 1)).toLocaleString()}</small>
        </div>
...
function renderCasinoRecord() {
    let st = getCasinoState();
    let rows = [
        ['今日已下注', `${st.wagered.toLocaleString()} / ${getCasinoDailyLimit().toLocaleString()}（依境界，每天 0 點重置）`],
        ['今日輸贏（估值）', `${st.net >= 0 ? '+' : ''}${st.net.toLocaleString()}`],
        ['今日最大收穫', st.bestText || '—'],
        ['累計切石', `${st.stones.toLocaleString()} 顆`],
        ['切出整朵異火', `${st.fires} 次`],
        ['累計擲骰', `${st.rolls.toLocaleString()} 把`],
        ['押中指定豹子', `${st.triples} 次`],
        ['擲骰單把最大淨贏', st.maxDiceWin.toLocaleString()]
    ];
    ...
    return `<table class="casino-table">${rows.map(r => `<tr><th>${r[0]}</th><td>${r[1]}</td></tr>`).join('')}</table>
        ...
        <p style="font-size: 0.75em; color: #6b7280;">※ 輸贏以估值計算：星允鐵 ${CASINO_VALUE.iron.toLocaleString()}／顆、異火碎片 ${CASINO_VALUE.fireShards.toLocaleString()}／片、紫裝 ${CASINO_VALUE.gear['紫色'].toLocaleString()}、橙裝 ${CASINO_VALUE.gear['橙色'].toLocaleString()}、整朵異火 ${CASINO_VALUE.fire.toLocaleString()}。</p>`;
}
